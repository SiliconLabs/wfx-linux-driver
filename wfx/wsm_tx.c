...
/*                       Functions                                        */
/*========================================================================*/
static int wfx_cmd_send(struct wfx_common *priv, struct wsm_buf *buf, void *arg,
			u8 cmd, long tmo);
		      ...
		      const u8 *conf_file_data)
{
	...
	wsm_cmd_lock(priv);
	/* DPD block. */
	wfx_cmd_len(wfx_arg, arg->req_part.Length);
	wfx_cmd(wfx_arg, (void *)(conf_file_data), arg->req_part.Length);
		  ...
		  size_t buf_size)
{
	...
	pr_debug("[WSM] >>> write MIB id 0x%.4X\n", mib_id);
...
int wsm_scan(struct wfx_common *priv, const struct wsm_scan *arg)
{
	...
	wfx_cmd(wfx_arg, &arg->scan_req.AutoScanParam,
		sizeof(WsmHiAutoScanParam_t));
...
int wsm_join(struct wfx_common *priv, WsmHiJoinReqBody_t *arg)
{
	...
	wfx_cmd_fl(wfx_arg, arg->DTIMPeriod); /* dtim_period */
		  ...
		  const struct wsm_update_ie	*arg)
{
	...
	if (ret < 0) {
		pr_debug("failed to sent: update_ie request");
		goto nomem;
	}
			...
			void *arg, u8 cmd, long tmo)
{
...

	{
		...
		if (msg_id >= NB_REQ_MSG || msg_type != 0)
			...
			pr_debug("[WSM] >>> send cmd id 0x%.4X\n", cmd);
	}
	...
	if (!ret && !priv->wsm_cmd.done) {
		...
		if (priv->bh_error) {
			...
			wfx_info(
				"[WFx request]: got stuck in _confirm. Reported command request = 0x%02x\n",
				priv->wsm_cmd.cmd);
			wake_up(&priv->bh_wq);
			ret = -ETIMEDOUT;
		}
