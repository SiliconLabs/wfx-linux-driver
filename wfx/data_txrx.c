...
static int wfx_handle_action_rx(struct wfx_common *priv, struct sk_buff *skb);
static const struct ieee80211_rate *
wfx_get_tx_rate(const struct wfx_common		*priv,
		const struct ieee80211_tx_rate	*rate);

/* ******************************************************************** */
/* TX queue lock / unlock                        */

static inline void wfx_tx_queues_lock(struct wfx_common *priv)
{
	int i;

	for (i = 0; i < 4; ++i)
		wfx_queue_lock(&priv->tx_queue[i]);
}

static inline void wfx_tx_queues_unlock(struct wfx_common *priv)
{
	int i;

	for (i = 0; i < 4; ++i)
		wfx_queue_unlock(&priv->tx_queue[i]);
}
...
static void tx_policy_build(const struct wfx_common *priv,
			    ...
			    struct ieee80211_tx_rate *rates, size_t count)
{
	...
	policy->defined = wfx_get_tx_rate(priv, &rates[0])->hw_value + 1;

	for (i = 0; i < count; ++i) {
		register unsigned rateid, off, shift, retries;

		rateid = wfx_get_tx_rate(priv, &rates[i])->hw_value;
...
void tx_policy_clean(struct wfx_common *priv)
{
	...
	for (idx = 0; idx < TX_POLICY_CACHE_SIZE; idx++) {
		...
		if (entry->policy.usage_count) {
			wfx_warn("Queues usage should be empty");
...
static int tx_policy_get(struct wfx_common *priv,
			 ...
			 size_t count, bool *renew)
{
	...
	if (list_empty(&cache->free)) {
		wfx_warn(
			"tx_policy_get: list is not empty, can not free the cache");
		spin_unlock_bh(&cache->lock);
...
static void tx_policy_put(struct wfx_common *priv, int idx)
{
	int usage, locked;
	struct tx_policy_cache *cache = &priv->tx_policy_cache;
...
static int tx_policy_upload(struct wfx_common *priv)
{
	struct tx_policy_cache *cache = &priv->tx_policy_cache;
	int i;
...
void tx_policy_upload_work(struct work_struct *work)
{
	struct wfx_common *priv =
		container_of(work, struct wfx_common, tx_policy_upload_work);
...
static int
wfx_tx_h_calc_link_ids(struct wfx_common	*priv,
		       struct wfx_txinfo	*t)
{
	if (t->sta && t->sta_priv->link_id) {
		...
		t->txpriv.link_id = wfx_find_link_id(priv, t->da);
		if (!t->txpriv.link_id)
			t->txpriv.link_id = wfx_alloc_link_id(priv, t->da);
		if (!t->txpriv.link_id) {
...
static void
wfx_tx_h_pm(struct wfx_common	*priv,
	    struct wfx_txinfo	*t)
{
...
static void
wfx_tx_h_calc_tid(struct wfx_common	*priv,
		  struct wfx_txinfo	*t)
{
...
static int
wfx_tx_h_crypt(struct wfx_common	*priv,
	       struct wfx_txinfo	*t)
{
...
static int
wfx_tx_h_align(struct wfx_common	*priv,
	       struct wfx_txinfo	*t,
	       WsmHiTxFlags_t		*flags)
{
	...
	flags->Offset = 1;
	wfx_debug_tx_align(priv);
	return 0;
}

static int
wfx_tx_h_action(struct wfx_common	*priv,
		struct wfx_txinfo	*t)
{
...
/* Add WSM header */
static WsmHiTxReq_t *
wfx_tx_h_wsm(struct wfx_common	*priv,
	     struct wfx_txinfo	*t)
{
	WsmHiTxReq_t *wsm;
	uint32 wsm_length = sizeof(WsmHiTxReq_t) - sizeof(uint32);
	...
	wsm->Header.MsgLen = __cpu_to_le16(t->skb->len);
	wsm->Header.s.t.MsgId = WSM_HI_TX_REQ_ID; /* useless __cpu_to_le16(WSM_HI_TX_REQ_ID); */
...
static int
wfx_tx_h_rate_policy(struct wfx_common	*priv,
		     struct wfx_txinfo	*t,
		     WsmHiTxReq_t	*wsm)
{
	...
	if (t->rate->flags & IEEE80211_TX_RC_MCS) {
		if (wfx_ht_greenfield(&priv->ht_info))
			wsm->ht_tx_parameters |=
				__cpu_to_le32(WSM_FRAME_FORMAT_GF_HT_11N);
		else
			wsm->ht_tx_parameters |=
				__cpu_to_le32(WSM_FRAME_FORMAT_MIXED_FORMAT_HT);
	}
#else
	...
	if (t->rate->flags & IEEE80211_TX_RC_SHORT_GI) {
		if (sgi_mode == 0)
			wsm->Body.HtTxParameters.ShortGi = 0;   /*LGI*/
		else
			wsm->Body.HtTxParameters.ShortGi = 1;   /*SGI*/
	}
	if (!sgi_control)
		if (wfx_ht_shortGi(&priv->ht_info))
	 ...
	 * Bit 4: 0 (BCC), 1(LDPC)
	 */
	if (t->tx_info->flags & IEEE80211_TX_CTL_LDPC) {
		if (ldpc_mode == 1)
			/*LDPC is enabled*/
			wsm->Body.HtTxParameters.FecCoding = 1;
		else
			/*LDPC is disabled (default)*/
			wsm->Body.HtTxParameters.FecCoding = 0;
	}

	if (!ldpc_control)
		if (wfx_ht_fecCoding(&priv->ht_info))
			wsm->Body.HtTxParameters.FecCoding = 1;
...
static bool
wfx_tx_h_pm_state(struct wfx_common	*priv,
		  struct wfx_txinfo	*t)
{
	int was_buffered = 1;
	...
	if (t->txpriv.raw_link_id && t->txpriv.tid < WFX_MAX_TID)
		was_buffered =
			priv->link_id_db[t->txpriv.raw_link_id -
					 1].buffered[t->txpriv.tid]++;
	    ...
	    struct sk_buff		*skb)
{
	...
	if (t.queue >= 4) {
		wfx_err("Wrong index into queue mapping %u", t.queue);
		goto drop;
	}

	ret = wfx_tx_h_calc_link_ids(priv, &t);
	if (ret)
		goto drop;
	...
	wfx_tx_h_pm(priv, &t);
	wfx_tx_h_calc_tid(priv, &t);
	ret = wfx_tx_h_crypt(priv, &t);
	if (ret)
		goto drop;
	ret = wfx_tx_h_align(priv, &t, &flags);
	if (ret)
		goto drop;
	ret = wfx_tx_h_action(priv, &t);
	if (ret)
		goto drop;
	wsm = wfx_tx_h_wsm(priv, &t);
	if (!wsm) {
		ret = -ENOMEM;
		goto drop;
	}
	wsm->Body.TxFlags = flags;
	ret = wfx_tx_h_rate_policy(priv, &t, wsm);
	if (ret)
		goto drop;
	...
	spin_lock_bh(&priv->ps_state_lock);
	{
		tid_update = wfx_tx_h_pm_state(priv, &t);
		BUG_ON(wfx_queue_put(&priv->tx_queue[t.queue],
				     t.skb, &t.txpriv));
	}
	spin_unlock_bh(&priv->ps_state_lock);
...
drop:
	wfx_skb_dtor(priv, skb, &t.txpriv);
	return;
}

/* ******************************************************************** */

static int wfx_handle_action_rx(struct wfx_common	*priv,
				struct sk_buff		*skb)
{
...
static int wfx_handle_pspoll(struct wfx_common	*priv,
			     struct sk_buff	*skb)
{
	struct ieee80211_sta *sta;
	...
	for (i = 0; i < 4; ++i) {
		if (wfx_queue_get_num_queued(&priv->tx_queue[i],
					     pspoll_mask)) {
			wfx_bh_wakeup(priv);
			drop = 1;
			break;
		       ...
		       WsmHiTxCnfBody_t		*arg)
{
	...
	if (queue_id >= 4) {
		wfx_err(" Wrong queue Id in transmit confirm %u", queue_id);
		return;
	}
	...
	if ((arg->Status == WSM_REQUEUE) &&
		...
		wfx_suspend_resume(priv, link_id, &suspend);
		...
		if (priv->bss_loss_state &&
		    arg->PacketId == priv->bss_loss_confirm_id) {
			if (arg->Status)
				/* Recovery failed */
				__wfx_cqm_bssloss_sm(priv, 0, 0, 1);
			else
				/* Recovery succeeded */
				__wfx_cqm_bssloss_sm(priv, 0, 1, 0);
		}
...
static void wfx_notify_buffered_tx(struct wfx_common *priv,
		  ...
		  const struct wfx_txpriv	*txpriv)
{
	...
	if (txpriv->rate_id != WF200_INVALID_RATE_ID) {
		wfx_notify_buffered_tx(priv, skb,
	       ...
	       struct sk_buff		**skb_p)
{
	...
	if (ieee80211_is_pspoll(frame->frame_control))
		if (wfx_handle_pspoll(priv, skb))
			goto drop;

	hdr->band = IEEE80211_BAND_2GHZ;
	...
	if (arg->RxFlags.Encryp) {
		...
		case WSM_RI_FLAGS_WAPI_ENCRYPTED:
			...
			break;
		default:
			wfx_warn("Unknown encryption type %d\n",
				 arg->RxFlags.Encryp);
			goto drop;
		}
	...
	if (arg->RxFlags.InAggr)
		wfx_debug_rxed_agg(priv);

	if (ieee80211_is_action(frame->frame_control) &&
	    (arg->RxFlags.MatchStationid)) {
		if (wfx_handle_action_rx(priv, skb))
			return;
	...
	if (ieee80211_is_auth(frame->frame_control))
		...
		grace_period = 1 * HZ;
	wfx_pm_stay_awake(&priv->pm_state, grace_period);
...
{
	struct wfx_common *priv =
		container_of(work, struct wfx_common, linkid_reset_work);

	if (!priv->action_linkid) {
		int temp_linkid;
		/* In GO mode we can receive ACTION frames without a linkID */
		temp_linkid = wfx_alloc_link_id(priv,
						&priv->action_frame_sa[0]);
		if (!temp_linkid)
			wfx_warn("Can no t allocate link id %d", temp_linkid);
		if (temp_linkid) {
...
int wfx_alloc_link_id(struct wfx_common *priv, const u8 *mac)
{
	...
	for (i = 0; i < WFX_MAX_STA_IN_AP_MODE; ++i) {
		if (!priv->link_id_db[i].status) {
			ret = i + 1;
			break;
		} else if (priv->link_id_db[i].status != WFX_LINK_HARD &&
	...
	if (ret) {
		struct wfx_link_entry *entry = &priv->link_id_db[ret - 1];
...
{
	struct wfx_common *priv =
		container_of(work, struct wfx_common, link_id_work);

	wsm_flush_tx(priv);
	wfx_link_id_gc_work(&priv->link_id_gc_work.work);
	wsm_unlock_tx(priv);
}
...
{
	struct wfx_common *priv =
		container_of(work, struct wfx_common, link_id_gc_work.work);
	/* WsmHiResetFlags_t reset = {
	 *  .ResetStat = false,
	 * };
	 */
	WsmHiMapLinkReqBody_t map_link = {
		.PeerStaId	= 0,
		.Flags		= 0,
	};
	unsigned long now = jiffies;
	...
	for (i = 0; i < WFX_MAX_STA_IN_AP_MODE; ++i) {
		...
		need_reset = false;
		mask = BIT(i + 1);
		if (priv->link_id_db[i].status == WFX_LINK_RESERVE ||
		    (priv->link_id_db[i].status == WFX_LINK_HARD &&
			...
			priv->link_id_map |= mask;
			if (priv->link_id_db[i].status != WFX_LINK_HARD)
				priv->link_id_db[i].status = WFX_LINK_SOFT;

			memcpy(map_link.MacAddr,
			       priv->link_id_db[i].mac,
			       ETH_ALEN);
			spin_unlock_bh(&priv->ps_state_lock);
			if (need_reset)
				wfx_unmap_link(priv, i + 1);
			map_link.PeerStaId = i + 1;
			wfx_map_link(priv, &priv->link_id_db[i], &map_link);
			next_gc = min(next_gc, WFX_LINK_ID_GC_TIMEOUT);
			spin_lock_bh(&priv->ps_state_lock);
		} else if (priv->link_id_db[i].status == WFX_LINK_SOFT) {
			ttl = priv->link_id_db[i].timestamp - now +
			      WFX_LINK_ID_GC_TIMEOUT;
			if (ttl <= 0) {
				need_reset = true;
				priv->link_id_db[i].status = WFX_LINK_OFF;
				priv->link_id_map &= ~mask;
				priv->sta_asleep_mask &= ~mask;
				priv->pspoll_mask &= ~mask;
				memset(map_link.MacAddr, 0, ETH_ALEN);
				spin_unlock_bh(&priv->ps_state_lock);
				wfx_unmap_link(priv, i + 1);
				spin_lock_bh(&priv->ps_state_lock);
			} else {
		...
		} else if (priv->link_id_db[i].status == WFX_LINK_RESET ||
			   priv->link_id_db[i].status == WFX_LINK_RESET_REMAP) {
			...
			if (status == WFX_LINK_RESET_REMAP) {
				memcpy(map_link.MacAddr,
				       ...
				       ETH_ALEN);
				map_link.PeerStaId = i + 1;
				wfx_map_link(priv, &priv->link_id_db[i],
					     &map_link);
				next_gc = min(next_gc, WFX_LINK_ID_GC_TIMEOUT);
			} else {
				need_reset = true;
