 ...
 * with endianness conversion but no bus lock
 */
static inline int __wfx_reg_read_32(struct wfx_common *priv,
				    u16 addr, u32 *val)
{
	__le32 tmp;
	int i = priv->hwbus_ops->hwbus_memcpy_fromio(priv->hwbus_priv, addr,
						     &tmp, sizeof(tmp));
				     ...
				     u16 addr, u32 val)
{
	...
	return priv->hwbus_ops->hwbus_memcpy_toio(priv->hwbus_priv, addr, &tmp,
						  sizeof(tmp));
}
 ...
 */
static inline int wfx_reg_read(struct wfx_common *priv, u16 addr, void *buf,
			       size_t buf_len)
{
	int ret;

	priv->hwbus_ops->lock(priv->hwbus_priv);
	ret = priv->hwbus_ops->hwbus_memcpy_fromio(priv->hwbus_priv, addr, buf,
						   buf_len);
	priv->hwbus_ops->unlock(priv->hwbus_priv);
	return ret;
}

static inline int wfx_reg_write(struct wfx_common *priv, u16 addr,
				const void *buf, size_t buf_len)
{
	int ret;

	priv->hwbus_ops->lock(priv->hwbus_priv);
	ret = priv->hwbus_ops->hwbus_memcpy_toio(priv->hwbus_priv, addr, buf,
						 buf_len);
	priv->hwbus_ops->unlock(priv->hwbus_priv);
	return ret;
}
			     ...
			     size_t buf_len, u32 prefetch, u16 port_addr)
{
	...
	if ((buf_len / 2) >= 0x1000) {
		wfx_err("Can't read more than 0xfff words.\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		wfx_err("Can't write address register.\n");
		goto out;
	}
	...
	ret = __wfx_reg_write_32(priv, WF200_CONFIG_REG_ID,
				 val32 | prefetch);
	if (ret < 0) {
		wfx_err("Can't write prefetch bit.\n");
		goto out;
	}
	...
	for (i = 0; i < 20; i++) {
		ret = __wfx_reg_read_32(priv, WF200_CONFIG_REG_ID, &val32);
		if (ret < 0) {
			wfx_err("Can't check prefetch bit.\n");
			goto out;
		}
		if (!(val32 & prefetch))
			break;
	...
	if (val32 & prefetch) {
		wfx_err("Prefetch bit is not cleared.\n");
		goto out;
	}

	/* Read data port */
	ret = priv->hwbus_ops->hwbus_memcpy_fromio(priv->hwbus_priv, port_addr,
						   buf, buf_len);
	if (ret < 0) {
		wfx_err("Can't read data port.\n");
		goto out;
	}
 ...
 */
static int wfx_indirect_write(struct wfx_common *priv, u32 addr,
			      const void *buf,
			      size_t buf_len, u16 port_addr)
{
	...
	if ((buf_len / 2) >= 0x1000) {
		wfx_err("Can't write more than 0xfff words.\n");
		return -EINVAL;
	}
	...
	/* Write data port */
	ret = priv->hwbus_ops->hwbus_memcpy_toio(priv->hwbus_priv, port_addr,
						 buf, buf_len);
	if (ret < 0) {
		wfx_err("Can't write data port.\n");
		goto out;
	}
...
/**
 * read and write registers for a 32bits data.
...
int wfx_data_read(struct wfx_common *priv, void *buf, size_t buf_len)
{
	...
	if (((unsigned long)buf & 3) && (buf_len > 4)) {
		wfx_err("buffer is not aligned to 4 bytes boundary.\n");
		return -EINVAL;
	}
	...
	while (retry <= MAX_RETRY) {
		ret = priv->hwbus_ops->hwbus_memcpy_fromio(
				priv->hwbus_priv,
				WF200_IN_OUT_QUEUE_REG_ID, buf,
				buf_len);
		if (!ret) {
			break;
		} else {
...
int wfx_data_write(struct wfx_common *priv, const void *buf, size_t buf_len)
{
	...
	while (retry <= MAX_RETRY) {
		ret = priv->hwbus_ops->hwbus_memcpy_toio(
				priv->hwbus_priv,
				WF200_IN_OUT_QUEUE_REG_ID,
				buf,
				buf_len);
		if (!ret) {
			break;
...
int wfx_sram_read(struct wfx_common *priv, u32 addr, void *buf, size_t buf_len)
{
	return wfx_indirect_read(priv, addr, buf, buf_len,
				 WF200_CONF_SRAM_PREFETCH_BIT,
				 WF200_SRAM_DPORT_REG_ID);
}

int wfx_ahb_read(struct wfx_common *priv, u32 addr, void *buf, size_t buf_len)
{
	return wfx_indirect_read(priv, addr, buf, buf_len,
				 WF200_CONF_AHB_PREFETCH_BIT,
				 WF200_AHB_DPORT_REG_ID);
}

int wfx_sram_write(struct wfx_common *priv, u32 addr, const void *buf,
		   size_t buf_len)
{
	return wfx_indirect_write(priv, addr, buf, buf_len,
				  WF200_SRAM_DPORT_REG_ID);
}

int wfx_ahb_write(struct wfx_common *priv, u32 addr, const void *buf,
		  size_t buf_len)
{
	return wfx_indirect_write(priv, addr, buf, buf_len,
				  WF200_AHB_DPORT_REG_ID);
 ...
 * It includes the conversion from Little Endian to the CPU endianness
 */
...
{
	return wfx_reg_read_32(priv, WF200_CONTROL_REG_ID, &val->U32CtrlReg);
}

int control_reg_write(struct wfx_common *priv, HiCtrlReg_t val)
{
	return wfx_reg_write_32(priv, WF200_CONTROL_REG_ID, val.U32CtrlReg);
}

/**
 * activate or not the IRQ sources in the device
 */
int wfx_irq_enable(struct wfx_common *priv, IrqEnable_t enable)
{
	...
	Config_reg.hif.IrqEnable = enable;
