...
/*========================================================================*/
MODULE_DESCRIPTION("mac80211 Silicon Laboratories WFx SDIO driver");
MODULE_LICENSE("GPL");
...
static struct wfx_platform_data_sdio *global_plat_data = &wfx_pi_hat_platform_data;
...
static size_t wfx_sdio_align_size(struct hwbus_priv *self, size_t size)
{
	...
	if(size == SDIO_BLOCK_SIZE){size = 2*SDIO_BLOCK_SIZE;}
...
static int __init wfx_sdio_init(void)
{
	...
	if (wfx_sdio_on(pdata)) {
		pr_err("wfx_wlan_sdio: wfx_sdio_on returned 1");
		ret = -1;
		goto err;
	}
	...
	if (ret) {
		pr_err("wfx_wlan_sdio: can't register driver %i\n", ret);
		goto err;
	}
...
static void __exit wfx_sdio_exit(void)
{
	...
	pr_info("wfx_wlan_sdio: wfx_sdio_exit\n");
...
static int sys_reboot_callback(struct notifier_block *self,
				unsigned long val,
				void *data){
	pr_info("notifier system called with %lu.\n",val);
	if (val == SYS_RESTART) {
		pr_debug("System is rebooting wfx_wlan_sdio will be unloaded. \n");
