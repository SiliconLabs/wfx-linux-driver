...
#define wfx_cmd_len(wfx_arg, val) __wfx_cmd(wfx_arg, val, u16, __le16, __cpu_to_le16)
#define wfx_cmd_data(wfx_arg, val) __wfx_cmd(wfx_arg, val, u32, __le32, __cpu_to_le32)
...
struct wsm_configuration {
	WsmHiConfigurationReqBody_t req_part;
	WsmHiConfigurationCnfBody_t cnf_part;
};
struct wsm_scan {
	WsmHiStartScanReqBody_t scan_req;
	WsmHiSsidDef_t *ssids;
	WsmHiChannelDef_t *ch;
};
...
int wsm_configuration(struct wfx_common *priv,
...
int wsm_remove_key(struct wfx_common *priv,
		   const WsmHiRemoveKeyReqBody_t *arg);
...
static inline int wsm_set_rcpi_rssi_threshold(struct wfx_common *priv,
...
{
	return wsm_write_mib(priv, WSM_MIB_ID_BEACON_FILTER_TABLE, ft, sizeof(*ft));
}
...
static inline int wsm_set_operational_mode(struct wfx_common *priv,
...
{
	return wsm_write_mib(priv, WSM_MIB_ID_TEMPLATE_FRAME, arg, sizeof(*arg));
}
static inline int wsm_set_protected_mgmt_policy(struct wfx_common *priv,
...
static inline int wsm_set_tx_rate_retry_policy(struct wfx_common *priv,
		WsmHiMibSetTxRateRetryPolicy_t *arg)
{
	size_t size = 4 + arg->NumTxRatePolicy * sizeof(WsmHiMibTxRateRetryPolicy_t);
	return wsm_write_mib(priv, WSM_MIB_ID_SET_TX_RATE_RETRY_POLICY, arg,
			     size);
}
static inline int wsm_set_ether_type_filter(struct wfx_common *priv,
		WsmHiMibEtherTypeDataFrameFilterSet_t *arg)
{
	return wsm_write_mib(priv, WSM_MIB_ID_SET_ETHERTYPE_DATAFRAME_FILTER,
		arg, sizeof(*arg));
}
static inline int wsm_set_udp_port_filter(struct wfx_common *priv,
...
static inline int wsm_set_override_internal_txrate(struct wfx_common *priv,
