                 ...
                 struct wsm_buf *buf)
{
    ...
    if (WARN_ON(((HiConfigurationCnf_t *)buf->begin)->Body.Status != WSM_STATUS_SUCCESS))
        return -EINVAL;
    return 0;
        ...
        struct wsm_buf *buf)
{
    ...
    memcpy(&arg->cnf_part, &((HiConfigurationCnf_t *)buf->begin)->Body, sizeof(arg->cnf_part));
        ...
        struct wsm_buf *buf)
{
    ...
    pr_info("wfx init done.\n"
        ...
        priv->wsm_caps.FirmwareLabel, priv->wsm_caps.FirmwareMajor,
        priv->wsm_caps.FirmwareMinor,
        priv->wsm_caps.FirmwareBuild,
        priv->wsm_caps.ApiVersion,
        *p_Capa);
        ...
        struct sk_buff **skb_p)
{
    ...
    memcpy(&rx , &((WsmHiRxInd_t *)buf->begin)->Body, sizeof(WsmHiRxIndBody_t));
     ...
     * Size of the Frame
     */
    hdr_len = (sizeof(WsmHiRxIndBody_t)+ sizeof(uint32_t)) - (sizeof(((WsmHiRxIndBody_t *)0)->Frame));
        ...
        struct wsm_buf *buf, __le16 msgLen)
{
    ...
    //u16 DataLen = msgLen - sizeof(HiMsgHdr_t) - sizeof (Body->IndicationId);
    HiRxStats_t *pRxStats;
    ...
    switch(Body->IndicationId)
    {
        ...
        case HI_GENERIC_INDICATION_ID_STRING:
            ...
            pr_info("%s", (char *)Body->IndicationData.RawData);
            break;
        case HI_GENERIC_INDICATION_ID_RX_STATS:
            pRxStats = &Body->IndicationData.RxStats;
            pr_info(
                "Receiving new Rx statistics :\n"
                "NbFrame %d, PER %d, throughput %dKbps/s\n"
                "NbFrame by rate :\n"
                ...
                "\t1Mbps %d, 2Mpbs %d, 5.5Mbps %d, 11Mpbs %d\n"
                "\t6Mbps %d, 9Mpbs %d, 12Mbps %d, 18Mpbs %d, 24Mbps %d, 36Mbps %d, 48Mbps %d, 54Mbps %d\n"
                "\tMCS0 %d, MCS1 %d, MCS2 %dHiGenericIndBody, MCS3 %d, MCS4 %d, MCS5 %d, MCS6 %d, MCS7 %d\n"
                "RSSI by rate:\n"
                "\t1Mbps %d, 2Mpbs %d, 5.5Mbps %d, 11Mpbs %d\n"
                "\t6Mbps %d, 9Mpbs %d, 12Mbps %d, 18Mpbs %d, 24Mbps %d, 36Mbps %d, 48Mbps %d, 54Mbps %d\n"
                "\tMCS0 %d, MCS1 %d, MCS2 %d, MCS3 %d, MCS4 %d, MCS5 %d, MCS6 %d, MCS7 %d\n"
                ...
                "\t6Mbps %d, 9Mpbs %d, 12Mbps %d, 18Mpbs %d, 24Mbps %d, 36Mbps %d, 48Mbps %d, 54Mbps %d\n"
                "\tMCS0 %d, MCS1 %d, MCS2 %d, MCS3 %d, MCS4 %d, MCS5 %d, MCS6 %d, MCS7 %d\n",
                pRxStats->NbRxFrame, pRxStats->PerTotal, pRxStats->Throughput,
                pRxStats->NbRxByRate[0], pRxStats->NbRxByRate[1], pRxStats->NbRxByRate[2], pRxStats->NbRxByRate[3],
                pRxStats->NbRxByRate[6], pRxStats->NbRxByRate[7], pRxStats->NbRxByRate[8], pRxStats->NbRxByRate[9], pRxStats->NbRxByRate[10], pRxStats->NbRxByRate[11], pRxStats->NbRxByRate[12], pRxStats->NbRxByRate[13],
                pRxStats->NbRxByRate[14], pRxStats->NbRxByRate[15], pRxStats->NbRxByRate[16], pRxStats->NbRxByRate[17], pRxStats->NbRxByRate[18], pRxStats->NbRxByRate[19], pRxStats->NbRxByRate[20], pRxStats->NbRxByRate[21],
                pRxStats->Per[0], pRxStats->Per[1], pRxStats->Per[2], pRxStats->Per[3],
                pRxStats->Per[6], pRxStats->Per[7], pRxStats->Per[8], pRxStats->Per[9], pRxStats->Per[10], pRxStats->Per[11], pRxStats->Per[12], pRxStats->Per[13],
                pRxStats->Per[14], pRxStats->Per[15], pRxStats->Per[16], pRxStats->Per[17], pRxStats->Per[18], pRxStats->Per[19], pRxStats->Per[20], pRxStats->Per[21],
                (s32)pRxStats->Rssi[0] / 100, (s32)pRxStats->Rssi[1] / 100, (s32)pRxStats->Rssi[2] / 100, (s32)pRxStats->Rssi[3] / 100,
                (s32)pRxStats->Rssi[6] / 100, (s32)pRxStats->Rssi[7] / 100, (s32)pRxStats->Rssi[8] / 100, (s32)pRxStats->Rssi[9] / 100, (s32)pRxStats->Rssi[10] / 100, (s32)pRxStats->Rssi[11] / 100, (s32)pRxStats->Rssi[12] / 100, (s32)pRxStats->Rssi[13] / 100,
                (s32)pRxStats->Rssi[14] / 100, (s32)pRxStats->Rssi[15] / 100, (s32)pRxStats->Rssi[16] / 100, (s32)pRxStats->Rssi[17] / 100, (s32)pRxStats->Rssi[18] / 100, (s32)pRxStats->Rssi[19] / 100, (s32)pRxStats->Rssi[20] / 100, (s32)pRxStats->Rssi[21] / 100,
                (s32)pRxStats->Snr[0] / 100, (s32)pRxStats->Snr[1] / 100, (s32)pRxStats->Snr[2] / 100, (s32)pRxStats->Snr[3] / 100,
                (s32)pRxStats->Snr[6] / 100, (s32)pRxStats->Snr[7] / 100, (s32)pRxStats->Snr[8] / 100, (s32)pRxStats->Snr[9] / 100, (s32)pRxStats->Snr[10] / 100, (s32)pRxStats->Snr[11] / 100, (s32)pRxStats->Snr[12] / 100, (s32)pRxStats->Snr[13] / 100,
                (s32)pRxStats->Snr[14] / 100, (s32)pRxStats->Snr[15] / 100, (s32)pRxStats->Snr[16] / 100, (s32)pRxStats->Snr[17] / 100, (s32)pRxStats->Snr[18] / 100, (s32)pRxStats->Snr[19] / 100, (s32)pRxStats->Snr[20] / 100, (s32)pRxStats->Snr[21]/ 100);
                break;
        default:
            pr_warn("wrong type in generic_ind\n");
            return -1;
    }
...
int wsm_handle_rx(struct wfx_common *priv, HiMsgHdr_t *wsm, struct sk_buff **skb_p)
{
    ...
    if (wsm_id == WSM_HI_TX_CNF_ID) {
        ...
        case HI_GENERIC_IND_ID:
            ret = wsm_generic_indication(priv, &wsm_buf, wsm->MsgLen);
            break;
        default:
