...
/*                       Functions                                        */
/*========================================================================*/
int wfx_queue_stats_init(struct wfx_queue_stats *stats, size_t map_capacity,
			 wfx_queue_skb_dtor_t skb_dtor,
			 struct wfx_common *priv);

int wfx_queue_init(struct wfx_queue *queue, struct wfx_queue_stats *stats,
		   u8 queue_id, size_t capacity, unsigned long ttl);

int wfx_queue_clear(struct wfx_queue *queue);

int wfx_queue_put(struct wfx_queue *queue, struct sk_buff *skb,
		  struct wfx_txpriv *txpriv);

int wfx_queue_get(struct wfx_queue *queue, u32 link_id_map, WsmHiTxReq_t **tx,
...
int wfx_queue_get_skb(struct wfx_queue *queue, u32 packet_id,
		      struct sk_buff **skb, const struct wfx_txpriv **txpriv);
...
bool wfx_queue_stats_is_empty(struct wfx_queue_stats *stats, u32 link_id_map);

size_t wfx_queue_get_num_queued(struct wfx_queue *queue, u32 link_id_map);
