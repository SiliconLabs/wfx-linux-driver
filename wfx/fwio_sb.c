 ...
 *
 * ImageSize: size of whole firmware file (including Cheksum), host init
 * Put: No. of bytes put into the download, init & updated by host
 * Get: No. of bytes read from the download, host init, device updates
 * HostStatus: Status of the Host (NOT_RDY, RDY, UPLOAD_PENDING,
 *                                 UPLOAD_COMPLETE) 2
 * NcpStatus: Status of the NCP  (NOT_RDY, RDY, DOWNLOAD_PENDING,
 *                                DOWNLOAD_COMPLETE, FAILURE) 6
 * Signature: signature of the firmware
 * fw_hash: Encrypted Firmware hash , used in SecureBoot
 * fw_ver: Encrypted Firmware version , used in SecureBoot
 */
struct sb_download_cntl_s {
	u32	ImageSize;
	u32	Put;
	u32	Get;
	u32	HostStatus;
	u32	NcpStatus;
	u8	Signature[FW_SIGNATURE_SIZE];
	u8	fw_hash[FW_HASH_SIZE];
	u8	fw_ver[FW_VERSION_SIZE];
};
 ...
 * @param fw_length firmware length in bytes
 *
 * @return 0 if success, else negative error code
 */
int wfx_secure_load_firmware_file(struct wfx_common *priv,
				  u8 *firmware, u32 fw_length)
{
	int ret = -EIO;
	u32 val32 = 0;
	HiCfgReg_t Config_reg;
	u32 put = 0, get = 0;
	/* 'Allocated' pointer - used for kmalloc/kfree operations */
	u8 *buf = NULL;
	/* Pointer on current 1k byte block to be uploaded */
	u8 *firmware_cur_block = NULL;
	/* Number of bytes already uploaded */
	u32 firmware_upload_count = 0;
	struct sb_download_cntl_s *const dca =
		(struct sb_download_cntl_s *)(WF200_DCA_BASE_ADDRESS);
	u32 fifo = (u32)(WF200_DOWNLOAD_FIFO_BASE_ADDRESS);
	const u8 fw_version[FW_VERSION_SIZE] = { 0x01, 0x00, 0x00, 0x00 };
	u32 firmware_blocks_size = fw_length - FW_SIGNATURE_SIZE -
					FW_HASH_SIZE;
	...
	do {
		...
		if (!timer_pending(&fwio_timer)) {
			wfx_err(
				"Timeout detected while waiting for NCP 'INFO_RDY' - %x\n",
				val32);
			ret = -ETIMEDOUT;
			goto error;
		}
	...
	if (!buf) {
		wfx_err("can't allocate secureboot label buffer.\n");
		ret = -ENOMEM;
		goto error;
	}
	/* Force 'null-terminating' character */
	buf[BOOTLOADER_LABEL_MAX_SIZE - 1] = 0;
	/* Read label from Shared RAM */
	wfx_sram_read(priv, WF200_SB_LABEL, buf, BOOTLOADER_LABEL_MAX_SIZE);

	wfx_info("FIRMWARE LOAD: START...\n");
	wfx_info("load secure Bootloader, SecureBoot label : %s\n", buf);
	...
	if (!buf) {
		wfx_err("can't allocate pte info buffer.\n");
		ret = -ENOMEM;
		goto error;
	}
	...
	do {
		...
		if (!timer_pending(&fwio_timer)) {
			wfx_err(
				"Timeout detected while waiting for NCP 'RDY' - %x\n",
				val32);
			ret = -ETIMEDOUT;
			goto error;
		}
	...
	if (keyset == 0xA0) {
		...
		if (memcmp(firmware, "KEYSETA0", 8) != 0) {
			wfx_err("Encrypted FW file is invalid (Wrong Key Set)\n");
			goto error;
		}
	} else {
		fw_sign_ofs = 0;
	 ...
	 */
	ret =
		wfx_sram_write(priv, GET_ADDR(
				       dca->Signature), &firmware[fw_sign_ofs],
			       FW_SIGNATURE_SIZE);
	if (ret < 0) {
		wfx_err("Can't write firmware signature !\n");
		goto error;
	}
	...
	ret =
		wfx_sram_write(priv, GET_ADDR(
				       dca->fw_hash), &firmware[fw_hash_ofs],
			       FW_HASH_SIZE);
	if (ret < 0) {
		wfx_err("Can't write firmware hash !\n");
		goto error;
	}
	 ...
	 */
	ret = wfx_sram_write(priv, GET_ADDR(
				     dca->fw_ver), fw_version, FW_VERSION_SIZE);
	if (ret < 0) {
		wfx_err("Can't write firmware hash !\n");
		goto error;
	}
	...
	for (firmware_upload_count = 0;
	     firmware_upload_count < firmware_blocks_size;
	     firmware_upload_count += SB_DOWNLOAD_BLOCK_SIZE) {
		...
		do {
			...
			if (!timer_pending(&fwio_timer)) {
				wfx_err(
					"Timeout detected while waiting for room in download fifo' - %x\n",
					get);
				ret = -ETIMEDOUT;
				goto error;
			}
			stats_wait_states_2++;
			 ...
			 (SB_DOWNLOAD_FIFO_SIZE - SB_DOWNLOAD_BLOCK_SIZE));
		stop_timer();

		/* Check NCP status is in [READY or DOWNLOAD_PENDING] */
		APB_READ(GET_ADDR(dca->NcpStatus), val32);
		if (!((val32 == NCP_READY) ||
		      (val32 == NCP_DOWNLOAD_PENDING))) {
			wfx_err("Unexpected NCP status = %x.\n", val32);
			ret = -ETIMEDOUT;
			goto error;
		}
		...
		ret = wfx_sram_write(priv,
				     ...
				     firmware_cur_block,
				     SB_DOWNLOAD_BLOCK_SIZE);
		if (ret < 0) {
			wfx_err("Can't write firmware block @ %d!\n",
	...
	do {
		...
		if (!timer_pending(&fwio_timer)) {
			wfx_err(
				"Timeout detected while waiting for NCP 'AUTH' - %x\n",
				val32);
			ret = -ETIMEDOUT;
			goto error;
		}
	...
	wfx_info("Download firmware file completed\n");

	return 0;

error:
	wfx_err("Error detected during secure firmware upload ...\n");
	 ...
	 */
	wfx_info("[SecureBoot]: state : %x\n", val32);
	wfx_sram_read_32(priv, WF200_MSG_ID, &val32);

	wfx_info("[SecureBoot]: reported id = %x\n", val32);
	wfx_sram_read_32(priv, WF200_ERROR_ID, &val32);

	switch (val32) {
	case INVALID_SEC_TYPE:
		wfx_info(
			"[SecureBoot]: reported error id (%x) = Invalid Section Type (may be caused by a wrong encryption)\n",
			val32);
		break;
	case SIG_VERIF_FAILED:
		wfx_info(
			"[SecureBoot]: reported error id (%x) = Signature Verification Failed\n",
			val32);
		break;
	case AES_CTRL_KEY:
		wfx_info(
			"[SecureBoot]: reported error id (%x) = AES Control Key Not Initialized\n",
			val32);
		break;
	case ECC_PUB_KEY:
		wfx_info(
			"[SecureBoot]: reported error id (%x) = ECC Public Key Not Initialized\n",
			val32);
		break;
	case MAC_KEY:
		wfx_info(
			"[SecureBoot]: reported error id (%x) = MAC Key Not Initialized\n",
			val32);
		break;
	default:
		wfx_info("[SecureBoot]: reported error id = %x\n", val32);
		break;
	}
	return ret;
}
