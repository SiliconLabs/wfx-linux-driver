...
/*========================================================================*/
MODULE_DESCRIPTION("Core module for WFx SPI and SDIO devices");
MODULE_LICENSE("GPL");
...
static int wfx_wup_gpio_num = 12;

module_param(wfx_wup_gpio_num, int, 0644);
MODULE_PARM_DESC(wfx_wup_gpio_num, "gpio number for the WUP pin");

int wfx_power_mode = WSM_OP_POWER_MODE_ACTIVE;

module_param(wfx_power_mode, int, 0644);
MODULE_PARM_DESC(wfx_power_mode,
		 "WSM power mode.  0 == RF always active (default), 1 == allow doze (RF off), 2 == allow quiescent (lowest energy mode)");

int sgi_control = 1; /* default, sgi is controlled by mac80211 */

module_param(sgi_control, int, 0644);
MODULE_PARM_DESC(sgi_control,
		 "Guard Interval mode. 0 == sgi controlled by wfx driver, "
		 "1 ==  sgi controlled by mac80211 (default)");

int ldpc_control = 1; /* default, LDPC is controlled by mac80211 */

module_param(ldpc_control, int, 0644);
MODULE_PARM_DESC(ldpc_control,
		 "Fec Coding mode. 0 == ldpc controlled by wfx driver, "
		 "1 == ldpc controlled by mac80211 (default)");

int sgi_mode = 1; /* default, SGI is enabled */

module_param(sgi_mode, int, 0644);
MODULE_PARM_DESC(sgi_mode,
		 "sgi. 0 == disabled , "
		 "1 == enabled (default)");

int ldpc_mode = 0; /* default, LDPC is disabled */
...
static const struct ieee80211_ops wfx_ops = {
	.start			= wfx_start,
	.stop			= wfx_stop,
	.reconfig_complete	= wfx_reconfig_complete,
					    ...
					    const bool	hif_clkedge)
{
	...
	priv->wup_gpio_num = wfx_wup_gpio_num;
	if (wfx_power_mode == WSM_OP_POWER_MODE_QUIESCENT) {
		/* to support this power mode we must use the WUP pin
		 * then we configure the host gpio for this WUP signal here */
		if (gpio_request(wfx_wup_gpio_num, "wfx_wlan_wup")) {
			wfx_err("error while requesting gpio #%d.\n", wfx_wup_gpio_num);
			ieee80211_free_hw(hw);
			return NULL;
		}
		if (gpio_direction_output(wfx_wup_gpio_num, 0)) {
			wfx_err("error while setting gpio #%d in output mode.\n",
				wfx_wup_gpio_num);
			ieee80211_free_hw(hw);
			return NULL;
		}
	...
	INIT_WORK(&priv->restart_work, wfx_core_restart); /*For FSM*/
...
static int wfx_register_common(struct ieee80211_hw *dev)
{
	...
	if (err) {
		wfx_err("Cannot register device (%d).\n",
			err);
#ifdef CONFIG_PM
...
static void wfx_free_common(struct ieee80211_hw *dev)
{
	...
	if (wfx_power_mode == WSM_OP_POWER_MODE_QUIESCENT) {
		gpio_set_value(wfx_wup_gpio_num, 0);
		msleep(10);
		gpio_free(wfx_wup_gpio_num);
	}
}
		   ...
		   bool sdio, bool hif_clkedge)
{
	...
	*core = priv;
	...
	wfx_info("INEO DEVICE INIT:  START...\n");

	err = wfx_load_firmware(priv);
	if (err)
		goto err2;
	wfx_info("FIRMWARE LOAD: END\n");
	msleep(100);

	if (wait_event_interruptible_timeout(priv->wsm_startup_done,
					     priv->firmware_ready,
					     10 * HZ) <= 0) {
		/* error : no start_up indication received at timeout end */
		HiCtrlReg_t CtrlReg;
		int ret;

		ret = control_reg_read(priv, &CtrlReg);
		if (ret < 0)
			wfx_err("startup timeout: can't read control register.\n");
		wfx_err(
			"Timeout waiting on device startup; HIF control_reg=0x%x\n",
			CtrlReg.U32CtrlReg);
		err = -ETIMEDOUT;
		goto err2;
	}
	...
	wfx_info("Firmware is wake up, configure it\n");
	wfx_send_pds(priv);
	wfx_info("send PDS configuration\n");
	...
	if (wfx_power_mode == WSM_OP_POWER_MODE_QUIESCENT) {
		...
		control_reg_write(priv, Control_reg);
		priv->sleep_activated = true;
		wfx_info(
			"lowest power mode activated. GPIO_WUP pin 'mode' info must be set to 'func' in the .pds file\n");
	}
...
void wfx_core_release(struct wfx_common *self)
{
	...
	wfx_irq_enable(self, IRQS_DISABLED);
