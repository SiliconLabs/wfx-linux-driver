...
static int wfx_status_show(struct seq_file *seq, void *v)
{
	...
	seq_puts(seq,   "wfx Wireless LAN driver status\n");
	...
	seq_printf(seq, "Firmware:   %s %d %d.%d\n",
		   ...
		   priv->wsm_caps.FirmwareVersion.FonctionalityNumber,
		   priv->wsm_caps.FirmwareVersion.MajorNumber,
		   priv->wsm_caps.FirmwareBuildNumber);
	seq_printf(seq, "FW API:     %d\n",
			priv->wsm_caps.FirmwareApiVer.HIApiVersion);
	...
	if (priv->join_status == WFX_JOIN_STATUS_STA) {
		...
		seq_printf(seq, "Preamble:   %s\n",
			   wfx_debug_preamble[priv->association_mode.PreambleType]);
...
{
	int ret = -ENOMEM;
	struct wfx_debug_priv *d = kzalloc(sizeof(struct wfx_debug_priv),
			GFP_KERNEL);
	priv->debug = d;
	if (!d)
		return ret;
