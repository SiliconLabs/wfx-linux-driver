    ...
    for (i = 0; i < TX_POLICY_CACHE_SIZE; ++i) {
        ...
        if (src->retry_count && !src->uploaded) {
            WsmHiMibTxRateRetryPolicy_t *dst =
                &arg.TxRateRetryPolicy + arg.NumTxRatePolicy*sizeof(WsmHiMibTxRateRetryPolicy_t);
            dst->PolicyIndex = i;
...
int wfx_map_link(struct wfx_common *priv, struct wfx_link_entry *link_entry, const WsmHiMapLinkReqBody_t *link)
{
    int ret;
            ...
            WsmHiTxReq_t *wsm)
{
    ...
    if (t->rate->flags & IEEE80211_TX_RC_GREEN_FIELD) {
        wsm->Body.HtTxParameters.FrameFormat = WSM_FRAME_FORMAT_GF_HT_11N;
    } else {
        /*HT mixed is used*/
        wsm->Body.HtTxParameters.FrameFormat = WSM_FRAME_FORMAT_MIXED_FORMAT_HT;
    }
#endif
    ...
    if (t->rate->flags & IEEE80211_TX_RC_SHORT_GI) {
        if(sgi_ctl==0) {
            wsm->Body.HtTxParameters.ShortGi = 0;
        } else {
    ...
    if (t->tx_info->flags & IEEE80211_TX_CTL_LDPC) {
        if(ldpc_ctl==1) {
            wsm->Body.HtTxParameters.FecCoding = 1; /*LDPC is enabled*/
        } else {
            wsm->Body.HtTxParameters.FecCoding = 0; /*LDPC is disabled (default)*/
        }
    }
        ...
        struct sk_buff **skb_p)
{
    ...
    /*
     * In 802.11n, short guard interval "Short GI" is introduced while the default value is 400ns.
...
void wfx_link_id_gc_work(struct work_struct *work)
{
    ...
    for (i = 0; i < WFX_MAX_STA_IN_AP_MODE; ++i) {
        ...
        if (priv->link_id_db[i].status == WFX_LINK_RESERVE ||
            ...
            memcpy(map_link.MacAddr, priv->link_id_db[i].mac, ETH_ALEN);
            spin_unlock_bh(&priv->ps_state_lock);
            if (need_reset) {
