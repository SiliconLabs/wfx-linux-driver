...
static int wfx_scan_start(struct wfx_common *priv, struct wsm_scan *scan)
{
	...
	wiphy_dbg(priv->hw->wiphy, "[SCAN] hw req, type %d, %d channels, flags: Fbg %d, Split %d, Pre %d, TxMod %d.\n",
			  scan->scan_req.ScanType, scan->scan_req.NumOfChannels, scan->scan_req.ScanFlags.Fbg,
			  scan->scan_req.ScanFlags.Split, scan->scan_req.ScanFlags.Pre, scan->scan_req.ScanFlags.TxMod);

	for (i = 0; i < scan->scan_req.NumOfChannels; ++i)
		tmo += (20*(scan->ch[i].MaxChannelTime)) + 10;
...
void wfx_scan_work(struct work_struct *work)
{
	...
	struct wsm_scan scan = {
		.scan_req.ScanType = WSM_SCAN_TYPE_FG,
	...
	if (!priv->scan.req || (priv->scan.curr == priv->scan.end)) {
		...
		for (it = priv->scan.curr + 1, i = 1;
		     it != priv->scan.end && i < WSM_API_CHANNEL_DEF_SIZE;
		     ++it, ++i) {
		...
		if (priv->join_status == WFX_JOIN_STATUS_STA) {
			scan.scan_req.ScanType = WSM_SCAN_TYPE_BG;
			scan.scan_req.ScanFlags.Fbg = 1;	/* WSM_SCAN_FLAG_FORCE_BACKGROUND */
		}
		scan.ch = kzalloc(
			sizeof(WsmHiChannelDef_t) * (it - priv->scan.curr),
			GFP_KERNEL);
		if (!scan.ch) {
		...
		for (i = 0; i < scan.scan_req.NumOfChannels; ++i) {
			scan.ch[i].ChannelNumber = priv->scan.curr[i]->hw_value;
...
void wfx_probe_work(struct work_struct *work)
{
	...
	ch[0].ChannelNumber = priv->channel->hw_value;
