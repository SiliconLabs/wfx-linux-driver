...
typedef enum { WLAN_NOT_READY	= 0,
	       WLAN_READY	= 1 } WlanRdy_t;

typedef enum { WLAN_WAKEDOWN	= 0,
	       WLAN_WAKEUP	= 1 } WlanWup_t;
...
#define WF200_CTRL_NEXT_LEN_MASK    (0x00000FFF)

/* Configuration register fields may have different meaning when in SPI or SDIO*/
typedef union HiCtrlReg_t_s {
	u32 U32CtrlReg;
	struct __attribute__((__packed__)) {
		u8 next_length_7_0;             /*[11:0]  READ ONLY Use Mask WF200_CTRL_NEXT_LEN_MASK*/
		u8 next_length_11_8 : 4;        /* Next output queue item length */
...
typedef enum {
	...
	WLAN_RDY_ENABLED	= 2,    /* Wlan_ready Irq enabled */
