...
typedef enum {SDIO_CRC_CHECK=0, /* Normal CRC check on data */
              SDIO_NO_CRC_CHECK=1 /*Disable SDIO CRC check on data transfers
			     (CRC result assumed always correct). */
} SDIODisCRC_t;

typedef enum {DOUT_NEG_EDGE=0,
              DOUT_POS_EDGE=1
} ClkPosedge_t;

typedef enum {IRQS_DISABLED  =0, /* Both Irq's disabled */
              DATA_IRQ_ENABLED=1, /*Data Irq enabled */
              WLAN_RDY_ENABLED=2, /*Wlan_rd enabled */
	      IRQS_ENABLED   =3, /*Both Irq's enabled */
} IrqEnable_t;

typedef enum {DAT1_IRQ_ENABLE=0, /* Interrupt on SDIO DAT1 */
	      DAT1_IRQ_DISABLE=1
} SDIODat1Irq_t;


typedef enum {CPU_RUN=0,
	      CPU_RESET=1
} CpuRst_t;

typedef enum {CHANNEL_NOT_BUSY=0,
	      CHANNEL_BUSY=1
} PreFetch_t;

typedef enum {CPU_CLK_ENABLE=0,
	      CPU_CLK_DISABLE=1
} CpuClkDis_t;

typedef enum {QUEUE_MODE=0, /* Queue mode for HIF messages */
	      DIRECT_MODE=1 /* Direct mode for internal memory access */
} AccessMode_t;

typedef enum {MODE0_B1B0B3B2=0,   /*Mode0 (“00”) : 4 bytes are sent : B1,B0,B3,B2
