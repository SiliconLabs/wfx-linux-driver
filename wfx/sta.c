...
static int wfx_enable_beaconing(struct wfx_common *priv, bool enable);
...
MODULE_PARM_DESC(wfx_bssloss_mitigation,
			  ...
			  int init, int good, int bad)
{
	...
	if (tx) {
		...
		if (!skb)
			wfx_err("Failed to retrieve a nullfunc. ");
		if (skb)
		...
		u16 queue, const struct ieee80211_tx_queue_params *params)
{
	...
	if (queue < dev->queues) {
		memcpy(&old_uapsd_flags, &priv->uapsd_info,
		       sizeof(old_uapsd_flags));

		WSM_TX_QUEUE_SET(&priv->tx_queue_params, queue, 0, 0, 0);
		ret = wsm_set_tx_queue_params(priv,
					      &priv->tx_queue_params.params[
						      queue], queue);
		if (ret) {
			ret = -EINVAL;
			goto out;
		...
		if (priv->mode == NL80211_IFTYPE_STATION) {
			ret = wfx_set_uapsd_param(priv, &priv->edca);
			memcpy(&new_uapsd_flags, &priv->uapsd_info,
...
int wfx_set_pm(struct wfx_common *priv, const WsmHiSetPmModeReqBody_t *arg)
{
	...
	if (wfx_power_mode == WSM_OP_POWER_MODE_ACTIVE) {
		...
		struct ieee80211_key_conf *key)
{
	...
	if (cmd == SET_KEY) {
		...
		wfx_warn("Unhandled key command %d\n", cmd);
	}
...
/*Call this function when reconfiguration has completed
 *
 * If the reconfiguration is completed, it returns a message:
 * " device successfully recovered"
 *
 * If device failed to restart it will be in a different state
 * and WFx must be suspended
 *
 * Reconfiguration type:
 * @IEEE80211_RECONFIG_TYPE_RESTART: WFx restart
 * @IEEE80211_RECONFIG_TYPE_SUSPEND: WFx suspend
 */
			   ...
			   enum ieee80211_reconfig_type reconfig_type)
{
	...
	/* If WFx failed to restart it will be in a different state */
	if (priv->state == WFX_STATE_RESTARTED) {
		pr_info("WFx successfully recovered\n");
		priv->state = WFX_STATE_ON;
...
{
	struct wfx_common *priv =
		container_of(work, struct wfx_common, restart_work);

	ieee80211_stop_queues(priv->hw);

	switch (priv->state) {
	case WFX_STATE_ON:
		priv->state = WFX_STATE_RESTARTING;
		ieee80211_restart_hw(priv->hw);
		break;
	case WFX_STATE_OFF:
		wfx_warn("cannot restart a device that hasn't been started\n");
		break;
	case WFX_STATE_RESTARTING:
		break;
	case WFX_STATE_RESTARTED:
		priv->state = WFX_STATE_RESTARTING_CRASH;
	case WFX_STATE_RESTARTING_CRASH:
		wfx_warn("device will not restart");
		break;
	case WFX_STATE_TESTMODE:
		break;
	}
	mutex_unlock(&priv->conf_mutex);
}
...
void wfx_event_handler(struct work_struct *work)
{
	...
	list_for_each_entry(event, &list, link) {
		...
		case WSM_EVENT_IND_ERROR:
			wfx_err("Unhandled WSM Error from LMAC\n");
			break;
		...
		case WSM_EVENT_IND_BT_INACTIVE:
			wfx_warn("Unhandled BT INACTIVE from LMAC\n");
			break;
		case WSM_EVENT_IND_BT_ACTIVE:
			wfx_warn("Unhandled BT ACTIVE from LMAC\n");
			break;
		}
	}
	__wfx_free_event_queue(&list);
...
int wfx_send_pds(struct wfx_common *priv)
{
	...
	if (priv->hw_revision == WF200_HW_REV) {
		if (!priv->pds) {
			ret = request_firmware(&priv->pds, priv->pds_path,
					       priv->pdev);
			if (ret) {
				wfx_err("WFX : Can't load PDS file %s.\n",
					priv->pds_path);
				return ret;
			}
		}
		...
		for (ui8_DataBufferIndex = 0;
		     ui8_DataBufferIndex < ps_PDSDataBuffers->nb_buffers_used;
		     ui8_DataBufferIndex++) {
			char *pc_PDSMsg;
			pc_PDSMsg =
				ps_PDSDataBuffers->output_strings[
					ui8_DataBufferIndex];
			ui16_pc_PDSMsgLen = strlen(pc_PDSMsg) + 1;

			if (ui16_pc_PDSMsgLen < 1500) {
				cfg.req_part.Length = ui16_pc_PDSMsgLen;
				pc_PDSMsg[cfg.req_part.Length] = 0;
				ret = wsm_configuration(priv, &cfg, pc_PDSMsg);
			} else {
				wfx_err("WFX : PDS message too long");
			}
		}
...
int wfx_setup_mac(struct wfx_common *priv)
{
	...
	WsmHiMibMacAddresses_t mac;

	(void)memset(&mac, 0, sizeof(WsmHiMibMacAddresses_t));
	memcpy(mac.MacAddr0, &priv->mac_addr[0], WSM_API_MAC_ADDR0_SIZE);
	memcpy(mac.MacAddr1, &priv->mac_addr[1], WSM_API_MAC_ADDR1_SIZE);

	ret = wsm_write_mib(priv, WSM_MIB_ID_DOT11_MAC_ADDRESSES, &mac,
			    sizeof(WsmHiMibMacAddresses_t));
...
static void wfx_do_join(struct wfx_common *priv)
{
	...
	if (delayed_work_pending(&priv->join_timeout)) {
		wfx_warn("[STA] - Join request already pending, skipping..\n");
		wsm_unlock_tx(priv);
		return;
	}
			  ...
			  u32				changed)
{
		...
		if (changed &
			...
			if (info->assoc || info->ibss_joined) {
				...
				pr_debug(
					...
					priv->bss_params.OperationalRateSet,
					priv->association_mode.BasicRateSet);
				wsm_set_association_mode(priv,
							 &priv->association_mode);

				if (!info->ibss_joined) {
					wsm_keep_alive_period(priv,
							      30 /* sec */);
					wsm_set_bss_params(priv,
							   &priv->bss_params);
				...
				memset(&priv->association_mode, 0,
				       sizeof(priv->association_mode));
				memset(&priv->bss_params, 0,
				       sizeof(priv->bss_params));
			}
		}
	}
	...
	if (changed & (BSS_CHANGED_ASSOC |
		...
		if (!(prev_erp_info & WLAN_ERP_NON_ERP_PRESENT))
			priv->erp_info &= ~WLAN_ERP_USE_PROTECTION;
