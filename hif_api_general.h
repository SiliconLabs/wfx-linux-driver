...
enum wfx_hif_sl_encryption_type {
	NO_ENCRYPTION = 0,
	TX_ENCRYPTION = 1,
	RX_ENCRYPTION = 2,
	HP_ENCRYPTION = 3
...
struct wfx_hif_sl_msg_hdr {
	u32    seqnum:30;
	u32    encrypted:2;
} __packed;

struct wfx_hif_sl_msg {
	struct wfx_hif_sl_msg_hdr hdr;
	__le16 len;
	u8     payload[];
} __packed;
...
struct wfx_hif_sl_tag {
	u8     tag[16];
} __packed;

enum wfx_hif_sl_mac_key_dest {
...
struct wfx_hif_req_set_sl_mac_key {
	u8     otp_or_ram;
	u8     key_value[API_KEY_VALUE_SIZE];
} __packed;

struct wfx_hif_cnf_set_sl_mac_key {
	__le32 status;
} __packed;

enum wfx_hif_sl_session_key_alg {
...
struct wfx_hif_req_sl_exchange_pub_keys {
	u8     algorithm:2;
	u8     reserved1:6;
	u8     reserved2[3];
...
struct wfx_hif_cnf_sl_exchange_pub_keys {
	__le32 status;
} __packed;
...
struct wfx_hif_cnf_sl_configure {
	__le32 status;
	u8     encr_bmp[32];
} __packed;

struct wfx_hif_req_prevent_rollback {
	__le32 magic_word; /* Return an error if not equal to 0x5C8912F3 */
} __packed;

struct wfx_hif_cnf_prevent_rollback {
	__le32 status;
} __packed;
...
struct wfx_hif_cnf_pta_settings {
	__le32 status;
} __packed;

struct wfx_hif_req_pta_priority {
	__le32 priority;
} __packed;

struct wfx_hif_cnf_pta_priority {
	__le32 status;
} __packed;

struct wfx_hif_req_pta_enable {
	u8     enable;
	u8     reserved[3];
} __packed;

struct wfx_hif_cnf_pta_enable {
	__le32 status;
} __packed;
